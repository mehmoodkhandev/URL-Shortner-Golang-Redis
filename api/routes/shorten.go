...
func ShortenURL(c *fiber.Ctx) error {
	...
	// Check if key (IP) already exists in Redis
	val, err := r2.Get(database.Ctx, c.IP()).Result()

	if err == redis.Nil {
		// New user/IP — initialize quota
		_ = r2.Set(database.Ctx, c.IP(), os.Getenv("API_QUOTA"), 30*time.Minute).Err()
	} else {
		// Key exists — decrement remaining requests
		valInt, _ := strconv.Atoi(val)
		if valInt <= 0 {
			limit, _ := r2.TTL(database.Ctx, c.IP()).Result()
			return c.Status(fiber.StatusServiceUnavailable).JSON(fiber.Map{
				"error":            "Rate Limit Exceeded",
				"rate_limit_reset": limit / time.Second, // seconds until reset
			})
		}
	}
	...
	if !helpers.RemoveDomainError(body.URL) {
		return c.Status(fiber.StatusServiceUnavailable).JSON(fiber.Map{
			"error": "You cant access this url",
		})
	}
